	...
	<?php
		...
		if (is_array($rows)) {
			foreach ($rows as $row) {
				...
				$link 		= JRoute::_( 'index.php?option=com_phocamaps&controller=phocamapsmap&task=edit&cid[]='. $row->id );
				?>
				<tr class="<?php echo "row$k"; ?>">
					...
					<td><?php echo '{phocamaps view=map|id='.$row->id.'} ' . JText::_('or'). '  {phocamaps view=link|id='.$row->id.'|text=Map}';?></td>
