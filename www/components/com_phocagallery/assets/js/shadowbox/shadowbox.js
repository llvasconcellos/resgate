...
Shadowbox.lib = function() {
	...
	return {
		...
		getPageXY : function(I) {
			var H = I.pageX
					|| (I.clientX + (document.documentElement.scrollLeft || document.body.scrollLeft));
			var J = I.pageY
					|| (I.clientY + (document.documentElement.scrollTop || document.body.scrollTop));
			return [ H, J ]
		},
...
(function() {
	...
	SL.getViewportHeight = function() {
		...
		if ((mode || client.isIE) && !client.isOpera) {
			h = client.isStrict ? document.documentElement.clientHeight
	...
	SL.getViewportWidth = function() {
		...
		var mode = document.compatMode;
		if (mode || client.isIE) {
			w = client.isStrict ? document.documentElement.clientWidth
			...
			var fn = function() {
				resizeContent(function() {
					...
					showBars(function() {
						...
						SL.get("shadowbox_body_inner").innerHTML = SL
								.createHTML(content.markup(dims));
	...
	var resizeContent = function(cb) {
		...
		if (cb) {
			...
			if (c) {
				...
				if (gallery[current].player == "img"
						&& options.handleOversize == "drag") {
					var top = parseInt(SL.getStyle(c, "top"));
					if (top + content.height < dims.inner_h) {
						SL.setStyle(c, "top", dims.inner_h - content.height
								+ "px")
					}
					var left = parseInt(SL.getStyle(c, "left"));
					if (left + content.width < dims.inner_w) {
						SL.setStyle(c, "left", dims.inner_w - content.width
								+ "px")
					}
				}
			}
	...
	var fixTop = function() {
		SL.get("shadowbox_container").style.top = document.documentElement.scrollTop
				+ "px"
	};
	var fixHeight = function() {
		SL.get("shadowbox_overlay").style.height = SL.getViewportHeight()
				+ "px"
	};
	var hasNext = function() {
		return gallery.length > 1
				&& (current != gallery.length - 1 || options.continuous)
	};
		...
		if (ltIE7) {
			...
			if (png && png.constructor == Array) {
				for ( var i = 0; i < png.length; ++i) {
					var el = SL.get(png[i]);
					if (el) {
						var match = SL.getStyle(el, "background-image").match(
								/url\("(.*\.png)"\)/);
						if (match) {
							SL
									.setStyle(
	...
	Shadowbox.loadSkin = function(skin, dir) {
		...
		skin = dir + skin + "/";
		document
				.write('<link rel="stylesheet" type="text/css" href="' + skin + 'skin.css">');
		document
				.write('<script type="text/javascript" src="' + skin + 'skin.js"><\/script>')
	};
	Shadowbox.loadLanguage = function(lang, dir) {
		if (!(/\/$/.test(dir))) {
			dir += "/"
		}
		document.write('<script type="text/javascript" src="' + dir
				+ "shadowbox-" + lang + '.js"><\/script>')
	};
			...
			if (gallery[i].player == "unsupported") {
				...
				if (match = RE.unsupported.exec(gallery[i].player)) {
					if (options.handleUnsupported == "link") {
						...
						var msg = SB.LANG.errors[s].replace(/\{(\d+)\}/g,
								function(m, i) {
									return a[i]
								});
						gallery[i].content = '<div class="shadowbox_message">'
								+ msg + "</div>"
					} else {
						r = true
					}
					...
					if (gallery[i].player == "inline") {
						...
						if (match) {
							...
							SB
									.raise("Cannot find element id for inline content")
						}
					}
				}
			}
	...
	var E = function(N, O, C) {
		if (N) {
			...
			var P = [
					...
					"height:" + O + "px",
					"width:" + C + "px",
					"cursor:" + (A.isGecko ? "-moz-grab" : "move"),
					"background-color:"
							+ (A.isIE ? "#fff;filter:alpha(opacity=0)"
									: "transparent") ];
			L.append(L.get("shadowbox_body_inner"), '<div id="' + J
					+ '" style="' + P.join(";") + '"></div>');
...
(function() {
	...
	Shadowbox.iframe = function(E, C) {
		this.id = E;
		this.obj = C;
		this.height = this.obj.height ? parseInt(this.obj.height, 10) : B
				.getViewportHeight();
		this.width = this.obj.width ? parseInt(this.obj.width, 10) : B
				.getViewportWidth()
	};
	Shadowbox.iframe.prototype = {
		...
		onLoad : function() {
			var C = (D.isIE) ? B.get(this.id).contentWindow
