	...
	<div id="phocagallery-categories-detail">
		<table width="800" style="margin-left:100px;">
			<?php for ($i = 0; $i < $countCategories; $i++) { ?>
				<tr>
					<td width="<?php echo $this->tmpl['categoriesboxwidth']; ?>">
						<a href="<?php echo $this->categories[$i]->link; ?>">
							<?php echo JHTML::_( 'image.site', $this->categories[$i]->linkthumbnailpath, '', '', '', str_replace('&raquo;', '-',$this->categories[$i]->title), 'style="border:0;"' ); ?>
						</a>
					</td>
					<td>
						<a href="<?php echo $this->categories[$i]->link; ?>" class="category'<?php echo $this->params->get( 'pageclass_sfx' ); ?>"><?php echo $this->categories[$i]->title_self; ?></a>
						<br/>
						<?php if(strlen(trim($this->categories[$i]->description))>0){
								echo $this->categories[$i]->description;
								echo '<br/>';
						} ?>
						<?php echo(JFactory::getDate($this->categories[$i]->date)->toFormat('%A, %d de %B de %Y'));?>
					</td>
				</tr>
				<tr>
					<td>&nbsp;</td>
